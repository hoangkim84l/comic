...
Class Stories extends MY_Controller
{
    ...
    function view()
    {
        ...
        $chapter_subs = $this->chapter_model->get_list($input_chapter);
        if(!empty($chapter_subs)){
            $chapter_subs_id = array();
            foreach($chapter_subs as $subs){
                $chapter_subs_id[] = $subs->id;
            }
            $this->db->where_in('story_id', $chapter_subs_id);
        }else{
