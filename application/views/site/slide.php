	...
	endforeach;
		...
		<div class="ml-auto hidden py-12 xl:inline-flex">
			<div class="flex flex-row items-center space-x-6 rounded-l-3xl bg-white/30 p-5 backdrop-blur-xl backdrop-brightness-150 transition-all">
				<?php
				foreach ($top2 as $row2) :
					if ($row2->status == 0) {
					} else {
				?>
						<img class="aspect-[16/9] w-32 animate-pulse cursor-pointer rounded-xl object-cover transition hover:scale-105" src="<?php echo $row2->image_link != '' ? base_url('upload/stories/' . $row2->image_link) :  base_url('upload/stories/default.jpg') ?>" alt="cafesua novel" />
					<?php
					}
				endforeach;

				foreach ($top3 as $row3) :
					if ($row3->status == 0) {
					} else {
					?>
						<img class="aspect-[16/9] w-32 cursor-pointer rounded-xl object-cover transition hover:scale-105" src="<?php echo $row3->image_link != '' ? base_url('upload/stories/' . $row3->image_link) :  base_url('upload/stories/default.jpg') ?>" alt="cafesua novel" />
				<?php
					}
				endforeach;
				?>
