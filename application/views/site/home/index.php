        ...
        <div class="col-lg-3 col-sm-6 mb-5">
          ...
          <a href="<?php echo site_url('xem-truyen/'.$row->slug.'-'.$row->id)?>">
            ...
            <div class="text-left fix-title-1-line">
              <?php 
                  $input_chapter = array();
                  $input_chapter['limit'] = array(1, 0);
                  $input_chapter['where'] = array('story_id' => $row->id);
                  $last_chap = $this->chapter_model->get_list($input_chapter);
                  foreach($last_chap as $val){
                      ?><a href="<?php echo site_url('truyen/'.$row->slug.'-'.$val->slug.'-'.$val->id)?>"> <?php echo $val->name ?></a><?php
                  }
              ?>
            </div>
           </article>
